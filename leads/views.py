...
from leads.forms import (LeadAttachmentForm, LeadCommentForm, LeadForm,
                         LeadListForm)
from leads.models import Lead,Product,Category,ItemList,SubCategory
...
def subCategory(request):
	...
	if cat_id:
		subd = Category.objects.filter(id=int(cat_id))
		cities = SubCategory.objects.filter(category_id__in=Subquery(subd.values('id'))).order_by('name')
		return render(request, 'dropdown.html', {'cities': cities})


def itemList(request):
	subc_id = request.GET.get('sub_category')
	subd = SubCategory.objects.filter(id=int(subc_id))
	cities = ItemList.objects.filter(sub_category_id__in=Subquery(subd.values('id'))).order_by('name')
	return render(request, 'dropdown.html', {'cities': cities})
